...
- 環境指的是 K8S 環境，包含 本地(kind、k3d)、雲端 K8S(EKS、GKE、LKE、AKS)、地端自建 (On-premises)、本地開發容器(DEVELOP_IMAGE container)
...
- 本地容器開發環境: 透過 project.env 設定的 DEVELOP_IMAGE 啟動 container，並且自動把 project 資料夾掛載進入 container 讓使用者可以快速啟動開發環境
- 本地 Kubernetes: 透過 Docker 啟動 kind(Kubernetes in Docker) or k3d(K3S in Docker)，快速啟動 Kubernetes/K3S 環境，並且透過 kubeconfig 連結 Kubernetes
  - local k8s(kind、k3d) 內每個 pvc 名稱都會對應到 project 資料夾底下的一個與 pvc 同名的資料夾 or 檔案
    - 透過 rancher 的 local-path-provisioner，將 environment/[環境名稱] 底下的 namespaces 資料夾掛載到 local-path-provisioner 的 hostPath (/opt/local-path-provisioner)，讓使用者可以透過在 namespace 底下建立 pvc，連結到與 pvc 相同名稱的檔案或資料夾，進而達到 Pod 內的資源同步
  - 透過 environment/[環境名稱]/init.sh，可以在 kind/k3d 啟動後執行對應的動作，例如：安裝 ingress、grafana、prometheus、...等等
- 遠端 Kubernetes:
  - 連結：透過 kubeconfig 連結遠端的 Kubernetes，讓使用者可以透過 telepresence 攔截 Pod 流量，並且連接到本地的容器開發環境，讓使用者可以透過本地的容器開發環境即時開發。
  - 建立：透過 Terraform 或 Ansible 建立 Kubernetes，然後透過 kubeconfig 連結 Kubernetes
