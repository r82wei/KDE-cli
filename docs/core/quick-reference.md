...
│   └── config
├── kind-config.template.yaml  # Kind 配置模板（可選，自訂配置用）
├── kind-config.yaml     # Kind 配置（Kind 環境，每次 start 從模板重新渲染，勿手動修改）
├── k3d-config.yaml      # K3D 配置（K3D 環境，每次 start 從模板重新渲染，勿手動修改）
├── pki/                 # PKI 憑證目錄
└── namespaces/          # 專案目錄
    └── <project_name>/
        ├── project.env      # 專案配置
...
> kind/k3d 配置模板中的 `${VAR}` 可引用 `kde.env`、`k8s.env`、`.env` 的變數；
> 修改這些檔案或模板後，`kde stop` + `kde start` 重啟環境即生效。
> 要調整 port 或掛載等配置，請改模板或環境變數檔，不要直接改生成的 `kind-config.yaml` / `k3d-config.yaml`。
