...
# 設定 KDE 版本
export KDE_VERSION=ay-v1.0.0-rc.6
# 設定 KDE cli 根目錄路徑
...
show_help() {
    ...
    echo "  code-server [-d] [-p port] [-n name] [-v dir] [-w dir]   啟動 code-server，-d 背景執行，-p 指定 port，-n 指定容器名稱(可同時啟動多個)，-v 指定掛載目錄或檔案(可重複指定多次，預設當前路徑)，-w 指定開啟的資料夾"
