...
- Expand environments to show projects by running `kde project ls` (tries `--env <env>` first, falls back to `kde use <env> && kde project ls`).

## Visibility of command outputs

- Background commands (e.g. listing projects) write to an Output Channel named "KDE Helper". Open it from: View -> Output -> select "KDE Helper".
- A progress indicator is shown in the window status area while loading projects.
- If loading projects fails, an error message is shown with the underlying stderr.
